...
const emptyQuery = { mode: "quick" as const, text: "", collectionIds: [], includeDescendants: false, libraryTagIds: [] };
...
equal(state.query.text, "治理", "TEST 1 continued input is preserved");
...
equal(filterLibrarySearchPapers(papers, { ...state.query, mode: "content" }).map((p) => p.id), [1, 2], "TEST 2 dimensions compose with AND");
equal(filterLibrarySearchPapers(papers, { ...emptyQuery, collectionIds: [10, 20], text: "Institutions" }).map((p) => p.id), [2], "TEST 3 multi-Collection de-dup");
equal(filterLibrarySearchPapers(papers, { ...emptyQuery, text: "Parent Record" }).map((p) => p.id), [3], "TEST 4 parent/child de-dup by paper id");
...
equal(sidebarQuery, { ...emptyQuery, collectionIds: [10], includeDescendants: true }, "TEST 5 sidebar auto token scope");
...
assert(suggestionIds.every((id) => id.startsWith("collection:") || id.startsWith("libraryTag:") || id.startsWith("paper:")), "TEST 6 has an external suggestion kind");
...
const runtimeQuery = { ...emptyQuery, text: "AI Governance" };
...
equal((await adapter.search({ ...emptyQuery, text: "Renamed Runtime" })).paperIds, [1], "TEST 8 runtime update");
...
const mixed = { ...emptyQuery, mode: "content" as const, text: "AI 治理 ESG 平台" };
...
assert(composing.isComposing === false && composing.query.text === "人工智能", "TEST 10 compositionend commits text");
