...
const emptyQuery = { freeTextQuery: "", fieldClauses: [], collectionIds: [], libraryTagIds: [] };
...
assert(filterLibrarySearchPapers([metadataOnlyPaper], { ...emptyQuery, freeTextQuery: "10.5555/metadata-only" }).length === 0, "excluded metadata is not full-text searchable");
...
equal(state.query.freeTextQuery, "", "TEST 1 token selection consumes transient input");
state = reduceLibrarySearchState(state, { type: "INPUT", text: "治理 平台" });
equal(state.query.freeTextQuery, "治理 平台", "TEST 1 continued input remains editable");
...
equal(filterLibrarySearchPapers(papers, { ...state.query }).map((p) => p.id), [2], "TEST 2 dimensions compose with AND");
equal(filterLibrarySearchPapers(papers, { ...emptyQuery, collectionIds: [10, 20], freeTextQuery: "Institutions" }).map((p) => p.id), [2], "TEST 3 multi-Collection de-dup");
equal(filterLibrarySearchPapers(papers, { ...emptyQuery, freeTextQuery: "Parent Record" }).map((p) => p.id), [3], "TEST 4 parent/child de-dup by paper id");
...
equal(sidebarQuery, { fieldClauses: [], freeTextQuery: "", collectionIds: [10], libraryTagIds: [] }, "TEST 5 sidebar auto token scope");
...
assert(suggestionIds.every((id) => id.startsWith("collection:") || id.startsWith("libraryTag:") || id.startsWith("paper:")), "TEST 6 only Library-local suggestions");
...
const runtimeQuery = { ...emptyQuery, freeTextQuery: "AI Governance" };
...
equal((await adapter.search({ ...emptyQuery, freeTextQuery: "Renamed Runtime" })).paperIds, [1], "TEST 8 runtime update");
...
const mixed = { ...emptyQuery, freeTextQuery: "AI 治理 ESG 平台" };
...
assert(composing.isComposing === false && composing.query.freeTextQuery === "人工智能", "TEST 10 compositionend commits text");
