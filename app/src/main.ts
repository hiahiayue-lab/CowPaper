...
interface LibraryItemMetadata {
  ...
  note: string | null;
  /** RC5: library-only identity overrides; canonical Paper remains untouched. */
  doiOverride?: string | null;
...
let librarySearchMatches = new Map<number, LibrarySearchMatch>();
...
async function loadLibraryData(view: "all" | "recent" | "unfiled" = libraryView) {
  ...
  try {
    ...
    const hasSearchScope = Boolean(searchQuery.queryText || searchQuery.fieldClauses?.length || searchQuery.collectionIds.length || searchQuery.libraryTagIds.length);
    ...
    libraryPapers = canonicalLibraryRows(libraryPapers);
...
/** Keep the visible Library projection one row per canonical papers.id. */
function canonicalLibraryRows(rows: readonly LibraryPaper[]): LibraryPaper[] {
  const byPaperId = new Map<number, LibraryPaper>();
  for (const row of rows) {
    if (!Number.isInteger(row.paper.id) || row.paper.id <= 0 || byPaperId.has(row.paper.id)) continue;
    byPaperId.set(row.paper.id, row);
  }
...
function libraryDoi(item: LibraryPaper): string {
  return item.effectiveDoi?.trim() || item.metadata?.doiOverride?.trim() || item.paper.normalizedDoi?.trim() || "";
}

function libraryUrl(item: LibraryPaper): string {
  return item.effectiveUrl?.trim() || item.metadata?.urlOverride?.trim() || item.paper.url?.trim() || "";
}
...
function libraryInlineFieldLabel(field: LibraryInlineField): string {
  return ({
    ...
    authors: "作者", abstract: "摘要", chineseAbstract: "中文摘要", note: "备注", doi: "DOI", url: "URL",
...
function libraryInlineCurrentValue(item: LibraryPaper, field: LibraryInlineField): string {
  switch (field) {
    ...
    case "doi": return libraryDoi(item);
    case "url": return libraryUrl(item);
  }
}
...
function libraryMetadataInput(item: LibraryPaper): LibraryItemMetadataInput {
  ...
  return {
    ...
    doiOverride: existing?.doiOverride ?? null,
    urlOverride: existing?.urlOverride ?? null,
  };
}
...
function beginLibraryInlineEdit(paperId: number, field: LibraryInlineField, button: HTMLElement): void {
  ...
  const save = async () => {
    ...
    if (field === "doi") metadata.doiOverride = trimmed || null;
    if (field === "url") metadata.urlOverride = trimmed || null;
    try {
...
interface BackendLibrarySearchHit {
  ...
  matched_fields?: LibrarySearchFieldKey[];
  snippets?: Partial<Record<LibrarySearchFieldKey, string>>;
}

function backendLibrarySearchText(query: LibrarySearchQuery): string {
  // The existing v19 FTS command accepts one all-field string. Field clauses
  // are sent as a conservative superset and then checked exactly against the
  // hydrated effective values below, so field semantics do not require a DB
  // schema change or a second index.
  return [query.queryText, ...(query.fieldClauses || []).map((clause) => clause.text)].filter(Boolean).join(" ");
}
  ...
  librarySearchAdapter = {
    async search(request): Promise<LibrarySearchResult> {
      ...
      const hits = await invoke<BackendLibrarySearchHit[]>("search_library", {
        queryText: backendLibrarySearchText(query),
      ...
      const source = canonicalLibraryRows(libraryPapers);
      ...
      for (const hit of hits) {
        if (seen.has(hit.paperId)) continue;
        ...
        if (!match) continue;
        seen.add(hit.paperId);
        paperIds.push(hit.paperId);
        papers.push(paper);
        const matchedFields = hit.matchedFields || hit.matched_fields;
        matches[hit.paperId] = {
          matchedFields: matchedFields?.length ? matchedFields : match.matchedFields,
          snippets: hit.snippets && Object.keys(hit.snippets).length ? hit.snippets : match.snippets,
        };
      }
      return {
        papers,
...
function librarySearchIndex() {
  return {
    ...
    papers: canonicalLibraryRows(libraryPapers).map(librarySearchPaper),
...
function renderLibrarySearchSuggestions(): void {
  ...
  const open = librarySearchState.phase !== "closed" && Boolean(query.queryText.trim() || query.fieldClauses?.length);
  ...
  const content = groups.filter(([, , items]) => items.length).map(([, label, items]) => `<section class="library-search-suggestion-group"><div class="library-search-suggestion-heading">${label}</div>${items.map((suggestion) => {
    ...
    const kind = suggestion.kind === "collection" ? "文集" : suggestion.kind === "libraryTag" ? "标签" : "论文";
    const evidence = suggestion.snippet ? `<span class="library-search-suggestion-snippet">${escapeHtml(suggestion.snippet)}</span>` : "";
  ...
  box.innerHTML = content || '<div class="library-search-empty">没有匹配的文集、标签或论文</div>';
}
...
async function executeLibrarySearch(): Promise<void> {
  ...
  try {
    ...
    if (query.queryText || query.fieldClauses?.length || query.collectionIds.length || query.libraryTagIds.length) {
      librarySearchResultIds = null;
      librarySearchMatches.clear();
      await loadLibraryData(libraryView);
    }
    ...
    librarySearchMatches = new Map(Object.entries(result.matches || {}).map(([id, match]) => [Number(id), match]));
...
function commitLibrarySearchInput(): void {
  ...
  librarySearchState = {
    ...librarySearchState,
    query: normalizeLibrarySearchQuery({ ...librarySearchState.query, queryText: parsed.fieldClauses.length ? parsed.queryText : raw, fieldClauses: nextClauses }),
    rawInput: parsed.fieldClauses.length ? parsed.queryText : raw,
    requestVersion: librarySearchState.requestVersion + 1,
  };
  input.value = librarySearchState.query.queryText;
}
...
function renderLibrarySearch(): void {
  ...
  const fieldTokens = (librarySearchState.query.fieldClauses || []).map((clause) => {
    const definition = LIBRARY_SEARCH_FIELDS.find((item) => item.key === clause.field);
    return `<span class="library-search-token field" data-token-kind="field" data-field="${escapeHtml(clause.field)}" title="${escapeHtml(`${definition?.label || clause.field}: ${clause.text}`)}"><span class="field-token-prefix">${escapeHtml(definition?.label || clause.field)}</span><span>${escapeHtml(clause.text)}</span><button type="button" class="library-search-token-remove" data-action="library-search-remove-token" data-token-kind="field" data-field="${escapeHtml(clause.field)}" data-field-text="${escapeHtml(clause.text)}" aria-label="移除字段 token">×</button></span>`;
  });
...
function renderLibraryMatchEvidence(paperId: number): string {
  const match = librarySearchMatches.get(paperId);
  if (!match?.matchedFields.length) return "";
  const fields = match.matchedFields
    .map((field) => LIBRARY_SEARCH_FIELDS.find((definition) => definition.key === field)?.label || field)
    .join(" · ");
  const snippet = Object.values(match.snippets)[0] || "";
  return `<span class="library-match-evidence"><span class="library-match-fields">命中 ${escapeHtml(fields)}</span>${snippet ? `<span class="library-match-snippet">${escapeHtml(snippet)}</span>` : ""}</span>`;
}
...
function handleLibrarySearchKeydown(event: KeyboardEvent): void {
  ...
  event.preventDefault();
  if (event.key === "Escape" && !librarySearchState.isComposing && next.query.queryText === "" && !(next.query.fieldClauses || []).length && hasLibrarySearchInput(librarySearchState.query)) {
    clearLibrarySearch();
    return;
  }
...
function renderLibrary() {
  ...
  list.innerHTML = visiblePapers.length ? visiblePapers.map((item) => {
    ...
    const cells: Record<LibraryColumn, string> = {
      title: `<span class="library-cell library-row-title" data-column="title" title="${escapeHtml(libraryEnglishTitle(item) + (chineseTitle ? ` · ${chineseTitle}` : ""))}"><span class="library-title-en"><span class="paper-symbol" aria-hidden="true"></span>${escapeHtml(libraryEnglishTitle(item))}</span>${chineseTitle ? `<span class="library-title-zh">${escapeHtml(chineseTitle)}</span>` : ""}${renderLibraryMatchEvidence(item.paper.id)}</span>`,
...
type ActiveLibraryDrag = "external-pdf" | "paper-row" | "column-header";
let activeLibraryDrag: ActiveLibraryDrag | null = null;

function claimLibraryDrag(kind: ActiveLibraryDrag): boolean {
  if (activeLibraryDrag && activeLibraryDrag !== kind) return false;
  activeLibraryDrag = kind;
  return true;
}

function releaseLibraryDrag(kind: ActiveLibraryDrag): void {
  if (activeLibraryDrag === kind) activeLibraryDrag = null;
}

function clearLibraryDragFeedback(): void {
  document.querySelectorAll(".membership-drop-hover, .column-drag-over, .pdf-drop-target").forEach((el) => {
    el.classList.remove("membership-drop-hover", "column-drag-over", "pdf-drop-target");
  });
}
...
class ColumnHeaderDrag {
  ...
  private onDown = (event: PointerEvent): void => {
    ...
    if (target.dataset.columnResize) {
      ...
      const columnEl = document.querySelector<HTMLElement>(".library-column-header[data-column=\"" + column + "\"]");
      const nextEl = document.querySelector<HTMLElement>(".library-column-header[data-column=\"" + next + "\"]");
      if (!columnEl || !nextEl) {
        releaseLibraryDrag("column-header");
        return;
      }
      this.state = { mode: "resize", column, next, startX: event.clientX, startY: event.clientY, startWidth: columnEl.getBoundingClientRect().width, nextWidth: nextEl.getBoundingClientRect().width, active: true };
      document.body.classList.add("is-resizing-library");
      event.preventDefault();
      return;
    }
  ...
  private onMove = (event: PointerEvent): void => {
    ...
    if (state.mode === "inspector") {
      ...
      libraryInspectorWidth = Math.min(maxWidth, Math.max(300, Math.round((state.startWidth || 300) - (event.clientX - state.startX))));
      applyLibraryLayoutMetrics();
      event.preventDefault();
      return;
    }
    if (state.mode === "resize") {
      ...
      const minDelta = LIBRARY_COLUMN_MIN[column] - (state.startWidth || 0);
      const maxDelta = (state.nextWidth || 0) - LIBRARY_COLUMN_MIN[next];
      const bounded = Math.max(minDelta, Math.min(maxDelta, delta));
      libraryColumnWidths[column] = Math.max(LIBRARY_COLUMN_MIN[column], Math.round((state.startWidth || 0) + bounded));
      libraryColumnWidths[next] = Math.max(LIBRARY_COLUMN_MIN[next], Math.round((state.nextWidth || 0) - bounded));
      applyLibraryLayoutMetrics();
      event.preventDefault();
      return;
    }
  ...
  private onUp = (event: PointerEvent): void => {
    ...
    releaseLibraryDrag("column-header");
    if (!state) return;
    if (state.mode === "inspector" || state.mode === "resize") {
      persistLibraryLayout();
      return;
    }
...
class ExternalPdfDrag {
  private pane = $("library-list-pane");

  install(): void {
    this.pane.addEventListener("dragenter", this.onDragEnter);
    this.pane.addEventListener("dragover", this.onDragOver);
    this.pane.addEventListener("dragleave", this.onDragLeave);
    this.pane.addEventListener("drop", this.onDrop);
    document.addEventListener("dragover", this.onDocumentDragOver);
    document.addEventListener("drop", this.onDocumentDrop);
    void listen("tauri://drag-over", (event) => this.onNativeDragOver(event.payload as { position?: { x: number; y: number } }));
    void listen("tauri://drag-leave", this.onNativeDragLeave);
    void listen("tauri://drag-drop", (event) => this.onNativeDrop(event.payload as { paths?: string[]; position?: { x: number; y: number } }));
  }
  ...
  private onDragEnter = (event: DragEvent): void => {
    if (!this.hasFiles(event) || !claimLibraryDrag("external-pdf")) return;
    event.preventDefault();
    setLibraryDropTarget(event.target);
  };

  private onDragOver = (event: DragEvent): void => {
    if (!this.hasFiles(event) || (activeLibraryDrag && activeLibraryDrag !== "external-pdf")) return;
    claimLibraryDrag("external-pdf");
    event.preventDefault();
    event.dataTransfer!.dropEffect = "copy";
    if (this.pane.contains(event.target as Node)) setLibraryDropTarget(event.target);
    this.setDiscoveryTarget(event.target);
  };

  private onDragLeave = (event: DragEvent): void => {
    if (!this.hasFiles(event)) return;
    const related = event.relatedTarget as Node | null;
    if (related && (this.pane.contains(related) || document.querySelector(".paper-card")?.contains(related))) return;
    clearLibraryDragFeedback();
    libraryDropActive = false;
    libraryDropTargetPaperId = null;
    renderLibraryDropState();
    releaseLibraryDrag("external-pdf");
  };

  private onDrop = (event: DragEvent): void => {
    if (!this.hasFiles(event)) return;
    event.preventDefault();
    void this.handleDomDrop(event);
  };

  private onDocumentDragOver = (event: DragEvent): void => {
    if (!this.hasFiles(event) || activeWorkspace !== "discovery" || (activeLibraryDrag && activeLibraryDrag !== "external-pdf")) return;
    claimLibraryDrag("external-pdf");
    event.preventDefault();
    event.dataTransfer!.dropEffect = "copy";
    this.setDiscoveryTarget(event.target);
  ...
  private onDocumentDrop = (event: DragEvent): void => {
    if (!this.hasFiles(event) || activeWorkspace !== "discovery") return;
    event.preventDefault();
    void this.handleDomDrop(event);
  };

  private setDiscoveryTarget(target: EventTarget | null): void {
    clearLibraryDragFeedback();
    const element = target instanceof Element ? target : null;
    const card = element?.closest<HTMLElement>(".paper-card[data-paper-id]");
    card?.classList.add("pdf-drop-target");
  }

  private targetAt(target: EventTarget | null): { kind: "library" | "discovery"; paperId: number | null } {
    const element = target instanceof Element ? target : null;
    const libraryRow = element?.closest<HTMLElement>(".library-paper-row");
    if (libraryRow) return { kind: "library", paperId: Number(libraryRow.dataset.paperId) };
    const card = element?.closest<HTMLElement>(".paper-card[data-paper-id]");
    if (card) return { kind: "discovery", paperId: Number(card.dataset.paperId) };
    return { kind: activeWorkspace === "library" ? "library" : "discovery", paperId: null };
  }

  private async handleDomDrop(event: DragEvent): Promise<void> {
    const files = droppedFilesFromDataTransfer(event.dataTransfer!);
    const target = this.targetAt(event.target);
    clearLibraryDragFeedback();
    libraryDropActive = false;
    libraryDropTargetPaperId = null;
    renderLibraryDropState();
    ...
    await this.process(files, target);
  }

  private async process(files: LibraryDroppedFile[], target: { kind: "library" | "discovery"; paperId: number | null }): Promise<void> {
    ...
    for (const file of files) {
      try {
        ...
        if (target.paperId != null) {
          ...
          setStatus("Discovery PDF 已导入：" + (result.attachment?.filename || file.name), "done");
        }
        succeeded++;
      } catch (error) {
        setStatus("Discovery PDF 失败：" + String(error), "error");
      }
    }
    if (succeeded) {
      await Promise.all([loadPapers(), loadLibraryData("all")]);
  ...
  private onNativeDragOver(payload: { position?: { x: number; y: number } }): void {
    if (activeLibraryDrag && activeLibraryDrag !== "external-pdf") return;
    if (!claimLibraryDrag("external-pdf")) return;
    ...
    if (target.kind === "library") setLibraryDropTarget(point);
    else this.setDiscoveryTarget(point);
  }
  ...
  private onNativeDrop(payload: { paths?: string[]; position?: { x: number; y: number } }): void {
    const target = this.targetAt(payload.position ? document.elementFromPoint(payload.position.x, payload.position.y) : null);
    const files = (payload.paths || []).filter((path) => path.toLowerCase().endsWith(".pdf")).map((path) => ({ name: path.split(/[\\/]/).pop() || path, path }));
    clearLibraryDragFeedback();
    libraryDropActive = false;
    renderLibraryDropState();
    releaseLibraryDrag("external-pdf");
    if (files.length) void this.process(files, target);
...
async function setupListeners() {
  ...
  document.addEventListener("click", async (ev) => {
    ...
    if (removeToken) {
      ...
      if (kind === "field") {
        const field = removeToken.dataset.field as LibrarySearchFieldKey | undefined;
        const fieldText = removeToken.dataset.fieldText || "";
        if (!field) return;
        librarySearchState = {
          ...librarySearchState,
          query: normalizeLibrarySearchQuery({
            ...librarySearchState.query,
            fieldClauses: (librarySearchState.query.fieldClauses || []).filter((clause) => !(clause.field === field && clause.text === fieldText)),
          }),
          requestVersion: librarySearchState.requestVersion + 1,
        };
        librarySearchAppliedQuery = null;
    ...
    if (t.closest("[data-action='select-reader-application']")) {
      await selectReaderApplication();
      return;
    }
    ...
    if (addLibrary) {
      const paperId = parseInt(addLibrary.dataset.paperId!, 10);
      const card = addLibrary.closest<HTMLElement>("[data-card-context]");
      const context = card?.dataset.cardContext || "manual";
      ...
      try {
        await invoke("add_paper_to_library", { paperId, collectionIds: [], tagIds: [], addedSource });
        libraryPaperIds.add(paperId);
        await Promise.all([loadPapers(), loadLibraryData("all")]);
        setStatus("已收录到文献库，可在 Inspector 中添加文集或 Library Tag", "done");
      } catch (err) { setStatus(`收录失败：${String(err)}`, "error"); }
      return;
    }
    ...
    if (collectionFilter) {
      ...
      librarySearchState = {
        ...
        rawInput: librarySearchState.query.queryText,
        activeSuggestionIndex: -1,
        requestVersion: librarySearchState.requestVersion + 1,
      };
      librarySearchAppliedQuery = null;
    ...
    if (openPdf) {
      ...
      try {
        ...
        await openPdfAttachment(Number(openPdf.dataset.attachmentId));
